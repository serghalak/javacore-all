...
/**
 * This program demonstrates compiling Java code in a Java program.
 */
public class CompilerDemo {
    void main(String[] args) throws Exception {
        ...
        String mainClassName = "v2ch08.compiler.Main";
        String worksheetName = args.length > 0 ? args[0] : "v2ch08/compiler/worksheet";
        ...
        for (Diagnostic<? extends JavaFileObject> d : diagnosticsListener.getDiagnostics())
            IO.println(d.getKind() + ": " + d.getMessage(null));
        fileManager.close();
        if (!result) {
     ...
     */
    JavaFileObject buildSource(String className, Path worksheet) throws IOException {
        ...
        for (String line : Files.readAllLines(worksheet)) {
            if (line.equals("```"))
                ...
                builder.append(line.replace("???", "IO.readln()")).append("\n");
            else {
                String[] fragments = line.split("`");
                for (int i = 0; i < fragments.length; i++) {
                    builder.append("IO.print(");
                    if (i % 2 == 0)
                        builder.append("\"").append(
                                fragments[i].replace("\\", "\\\\").replace("\"", "\\\""))
                                .append("\"");
                    else
                        builder.append(fragments[i]);
                    builder.append(");\n");
                }
                builder.append("IO.println()\n;");
            }
        }

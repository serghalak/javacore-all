...
class TypeLiteral<T> {
    ...
    /**
     * This constructor must be invoked from an anonymous subclass as new
     * TypeLiteral<. . .>(){}.
     */
    public TypeLiteral() {
...
class Formatter {
    ...
    public String formatFields(Object obj)
        ...
        for (Field f : obj.getClass().getDeclaredFields()) {
            ...
            Function<?, String> formatterForType = rules
                    .get(TypeLiteral.of(f.getGenericType()));
            if (formatterForType != null) {
                // formatterForType has parameter type ?. Nothing can be passed to its 
                // apply method. The cast makes the parameter type to Object so we can
                // invoke it.
...
public class TypeLiterals {
    ...
    void main() throws Exception {
        ...
        formatter.forType(new TypeLiteral<ArrayList<Character>>() { },
                lst -> "\"" + join("", lst) + "\"");
        IO.println(formatter.formatFields(new Sample()));
    }
}
